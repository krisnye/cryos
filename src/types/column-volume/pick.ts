...
/**
 * A column volume occupies a bounding box with size [0-size[0], 0-size[1], 0-size[2]].
 * The voxel at index 0 is centered at position [0.5, 0.5, 0.5] with bounds [0, 0, 0] -> [1, 1, 1].
 * 
 * We pick the closest voxel to the line.a that is pickable, working directly with the sparse
 * column representation without converting to dense format.
 * 
 * @param volume The column volume to pick from
...
export const pick = <T>(
    ...
    // Unlike DenseVolume.pick(), we check if columns exist before accessing voxels.
    ...
    while (iterations < maxIterations && currentDistance <= maxDistance) {
        ...
        if (voxelX >= 0 && voxelX < width && 
            ...
            const tileIdx = tileIndex(volume, voxelX, voxelY);
            ...
            if (columnInfo !== 0) {
                ...
                if (relativeZ >= 0 && relativeZ < length) {
                    ...
                    const voxel = volume.data.get(dataOffset + relativeZ);
                    
                    if (pickable(voxel)) {
                        const alpha = currentDistance / len;
                        return {
                            coordinates: [voxelX, voxelY, voxelZ],
                            alpha,
                            face: entryFace
                        };
                    }
