...
describe("toDenseVolume", () => {
    describe("empty volume", () => {
        it("should produce DenseVolume with all defaults for empty ColumnVolume", () => {
            const columnVolume: ColumnVolume<PhysicalVoxel> = {
                type: "column",
                size: [2, 2, 2],
            ...
            // All voxels should be default (0 for Material.Id)
            for (let i = 0; i < 8; i++) {
    ...
    describe("fully dense volume", () => {
        it("should produce DenseVolume with all voxels for fully dense ColumnVolume", () => {
            const data = createTypedBuffer(PhysicalVoxel.schema, 8);
            for (let i = 0; i < 8; i++) {
                data.set(i, PhysicalVoxel.pack(Material.ids.concrete));
            }
            ...
            for (let i = 0; i < 8; i++) {
                expect(result.data.get(i)).toBe(PhysicalVoxel.pack(Material.ids.concrete));
            }
        });
    });
    ...
    describe("sparse volume", () => {
        it("should produce DenseVolume with defaults for empty columns", () => {
            const data = createTypedBuffer(PhysicalVoxel.schema, 3);
            data.set(0, PhysicalVoxel.pack(Material.ids.concrete));
            data.set(1, PhysicalVoxel.pack(Material.ids.concrete));
            data.set(2, PhysicalVoxel.pack(Material.ids.concrete));
    ...
    describe("round-trip conversion", () => {
        it("should preserve non-empty voxels in DenseVolume → ColumnVolume → DenseVolume", () => {
            const original: DenseVolume<PhysicalVoxel> = {
                type: "dense",
                size: [2, 2, 3],
