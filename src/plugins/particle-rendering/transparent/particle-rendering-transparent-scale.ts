            ...
            create: (db) => {
                return () => {
                    ...
                    if (!pipeline && bindGroupLayout) {
                        pipeline = db.store.resources.transparentScalePipeline = createTransparentRenderPipeline(device, bindGroupLayout, shaderSourceScale, canvasFormat);
                    }
                    ...
                    const { buffer: sortedIndexBuffer } = getOrCreateSortedIndexBuffer(device, particleCount, db.store.resources.transparentScaleSortedIndexBuffer);
