            ...
            create: (db) => {
                return () => {
                    ...
                    if (!pipeline && bindGroupLayout) {
                        pipeline = db.store.resources.transparentScaleRotationPipeline = createTransparentRenderPipeline(device, bindGroupLayout, shaderSourceScaleRotation, canvasFormat);
                    }
                    ...
                    const { buffer: sortedIndexBuffer } = getOrCreateSortedIndexBuffer(device, particleCount, db.store.resources.transparentScaleRotationSortedIndexBuffer);
