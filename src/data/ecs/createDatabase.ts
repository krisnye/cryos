...
import { Archetype_deleteRow } from "./Archetype/deleteRow";
import { Table_getRowData } from "../Table/getRowData";
import { Table_updateRow } from "../Table/updateRow";
...
export function createDatabase(): Database<CoreComponents> {
    ...
    const getArchetype = <CC extends keyof CoreComponents>(componentNames: CC[]): Archetype<CoreComponents & { [K in CC]: CoreComponents[K]}> => {
        ...
        const archetype = createArchetype(componentSchemas as any, id, entityLocationTable);
    ...
    const selectEntity = (entity: Entity): CoreComponents | null => {
        ...
        return location.archetype >= 0 ? Table_getRowData(archetypes[location.archetype], location.row) : null;
    }
    ...
    const deleteEntity = (entity: Entity) => {
        ...
        if (location.archetype >= 0) {
            ...
            Archetype_deleteRow(archetype, location.row, entityLocationTable);
    ...
    const updateEntity = (entity: Entity, components: Partial<CoreComponents>) => {
        ...
        if (newArchetype !== currentArchetype) {
            ...
            const currentData = Table_getRowData(currentArchetype, currentLocation.row);
            ...
            Archetype_deleteRow(currentArchetype, currentLocation.row, entityLocationTable);
            const newRow = Table_addRow(newArchetype, { ...currentData, ...components });
            ...
            Table_updateRow(newArchetype, currentLocation.row, components);
        }
    }
    ...
    const database = {
        ...
        withExtension,
        simplifyTypes,
    } as const satisfies Database<CoreComponents, Extensions>;
    return database;
}
