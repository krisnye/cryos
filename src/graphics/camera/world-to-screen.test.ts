...
describe("worldToScreen", () => {
    ...
    it("should convert world position at origin to screen center", () => {
        const [screenX, screenY, depth] = Camera.worldToScreen([0, 0, 0], camera, canvasWidth, canvasHeight);
    ...
    it("should convert positive X world position to right side of screen", () => {
        const [screenX, screenY, depth] = Camera.worldToScreen([5, 0, 0], camera, canvasWidth, canvasHeight);
    ...
    it("should convert positive Y world position to top of screen", () => {
        const [screenX, screenY, depth] = Camera.worldToScreen([0, 5, 0], camera, canvasWidth, canvasHeight);
        expect(screenX).toBeCloseTo(canvasWidth / 2, 0);
    ...
    it("should handle positions behind camera", () => {
        const [screenX, screenY, depth] = Camera.worldToScreen([0, 0, 20], camera, canvasWidth, canvasHeight);
    ...
    it("should handle positions very close to camera", () => {
        const [screenX, screenY, depth] = Camera.worldToScreen([0, 0, 0.5], camera, canvasWidth, canvasHeight);
        expect(screenX).toBeCloseTo(canvasWidth / 2, 0);
        expect(screenY).toBeCloseTo(canvasHeight / 2, 0);
        expect(depth).toBeGreaterThanOrEqual(0);
        expect(depth).toBeLessThanOrEqual(1);
    ...
    it("should handle different canvas aspect ratios", () => {
        const [squareX, squareY, squareDepth] = Camera.worldToScreen([0, 0, 0], camera, 1000, 1000);
        const [wideX, wideY, wideDepth] = Camera.worldToScreen([0, 0, 0], camera, 2000, 1000);
    ...
    it("should return consistent depth values", () => {
        const [x1, y1, depth1] = Camera.worldToScreen([0, 0, 5], camera, canvasWidth, canvasHeight);
        const [x2, y2, depth2] = Camera.worldToScreen([0, 0, 5], camera, canvasWidth, canvasHeight);
        expect(depth1).toBeCloseTo(depth2, 6);
    });

    describe("depth calculation", () => {
        it("should return ~0.5 for position at near plane", () => {
            const [, , depth] = Camera.worldToScreen([0, 0, 0.1], camera, canvasWidth, canvasHeight);
            expect(depth).toBeCloseTo(0.5, 2);
        });

        it("should return 2 for position at far plane (behind camera)", () => {
            const [, , depth] = Camera.worldToScreen([0, 0, 100], camera, canvasWidth, canvasHeight);
            expect(depth).toBe(2);
        });

        it("should return ~0.5 for position at middle distance in front of camera", () => {
            const [, , depth] = Camera.worldToScreen([0, 0, 5], camera, canvasWidth, canvasHeight);
            expect(depth).toBeCloseTo(0.5, 2);
        });

        it("should return values between 0 and 1 for positions in range", () => {
            const [, , depth1] = Camera.worldToScreen([0, 0, 1], camera, canvasWidth, canvasHeight);
            const [, , depth2] = Camera.worldToScreen([0, 0, 2], camera, canvasWidth, canvasHeight);
            const [, , depth3] = Camera.worldToScreen([0, 0, 5], camera, canvasWidth, canvasHeight);
        ...
        it("should handle positions behind camera", () => {
            const [, , depth] = Camera.worldToScreen([0, 0, 20], camera, canvasWidth, canvasHeight);
            expect(depth).toBe(2);
        });

        it("should handle positions very close to camera", () => {
            const [, , depth] = Camera.worldToScreen([0, 0, 0.5], camera, canvasWidth, canvasHeight);
            expect(depth).toBeCloseTo(0.5, 2);
        });

        it("should be consistent for visible positions", () => {
            const worldPos: [number, number, number] = [2, 1, 5];
            const [screenX, screenY, depth] = Camera.worldToScreen(worldPos, camera, canvasWidth, canvasHeight);
            expect(depth).toBeGreaterThanOrEqual(0);
            expect(depth).toBeLessThanOrEqual(1);
