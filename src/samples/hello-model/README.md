...
- `hello-model-service.ts` - Store schema and service creation (~30 lines)
- `hello-model-application.ts` - UI component with two viewports (~60 lines)
...
The sample is automatically registered in the sample container. Run the dev server and select "Hello Model" from the samples list:

```bash
pnpm run dev
```

Then navigate to the local server URL and click on "Hello Model".
...
- **hello-model**: Shows basic 3D rendering with multiple viewports
