...
/**
 * Calculates face visibility flags for voxels in a static voxel chunk.
 * 
 * Each face of a voxel is marked as visible if there is no adjacent voxel on that side.
 ...
 * 
 * This function preserves any existing flags that are not part of the face visibility mask.
 * 
    ...
    for (let y = 0; y < chunk.size; y++) {
        for (let x = 0; x < chunk.size; x++) {
            ...
            for (let i = 0; i < tile.dataLength; i++) {
                ...
                let faceFlags = 0;
                if (!hasFront) faceFlags |= FRONT_FACE_VISIBLE;
                if (!hasRight) faceFlags |= RIGHT_FACE_VISIBLE;
                if (!hasBack) faceFlags |= BACK_FACE_VISIBLE;
                if (!hasLeft) faceFlags |= LEFT_FACE_VISIBLE;
                if (!hasTop) faceFlags |= TOP_FACE_VISIBLE;
                if (!hasBottom) faceFlags |= BOTTOM_FACE_VISIBLE;
                
                // Preserve existing flags that are not part of the face visibility mask
                const existingFlags = voxel.flags & ~ALL_FACES_VISIBLE_MASK;
